...
app = modal_app                 # 👈 NEW — gives Modal the expected name

from fastapi import Response

# 2️⃣  expose the endpoint
@modal_app.function(image=modal_image)            # or use gpu=… if you want GPU here
@modal.fastapi_endpoint(method="POST")      # <- renamed decorator
def tts_web_endpoint(text: str):
    if not text.strip():
        raise ValueError("text empty")

    # 1️⃣ run the TTS on the GPU container
    out_path = InferenceTTS().generate.remote(input_text=text)

    # 2️⃣ translate to a path inside the Volume
    rel_path = os.path.relpath(out_path, "/data")   # e.g. audio/generations/output_....wav

    # 3️⃣ stream bytes directly from the Volume
    wav_bytes = modal_volume.read_file(rel_path)    # <- no FileNotFoundError now
